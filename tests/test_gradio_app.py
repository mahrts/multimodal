...
from gradio_app.gradio_app import create_chat_interface, ChatSessionWithTracing
...
async def test_chat_with_gemini_calls_moderation():
    ...
    with patch('gradio_app.gradio_app.check_content_safety') as mock_moderation:
        # Mock successful moderation
        mock_moderation.return_value = (True, "Content passed moderation", "text/plain")

        # Mock the agent run
        with patch('gradio_app.gradio_app.customer_agent.run') as mock_agent:
